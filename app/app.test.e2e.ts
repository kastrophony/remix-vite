...
describe(`SPA (${mode})`, () => {
  ...
  it("loads a client route directly through the Vite history fallback", async (t) => {
    ...
    await page.getByRole("heading", { name: "URLs in, rendered UI out" })
      .waitFor();

    assert.equal(new URL(page.url()).pathname, "/about");
    assert.equal(
      await page.getByRole("link", { name: "About" }).getAttribute(
        "aria-current",
      ),
      "page",
    );
  ...
  it("navigates between routes and cancels a superseded load", async (t) => {
    ...
    await page.goto("/");
    await page.getByRole("heading", { name: "A client-only Remix app" })
      .waitFor();
  ...
  it("pushes new form destinations and replaces submissions to the active URL", async (t) => {
    ...
    assert.equal(
      await page.getByRole("heading", { name: "Hello, Ada!" }).count(),
      1,
    );
