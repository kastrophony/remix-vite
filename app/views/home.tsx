...
export function HomePage() {
  return () => (
    <main className="flex min-h-screen items-center justify-center bg-surface-0 px-6 py-12 text-text-primary font-sans text-sm leading-normal antialiased">
      <div className="flex w-full max-w-[820px] flex-col items-center gap-[72px]">
        <Masthead />
        <Columns />
        <Footer />
      </div>
...
function Masthead() {
  return () => (
    ...
    >
      <p className="m-0 text-center text-sm font-bold uppercase tracking-[0.1em] text-text-primary">
        Welcome to
      </p>
      <RemixWordmarkHero />
    </section>
...
function Columns() {
  return () => (
    <section
      ...
      className="flex w-auto items-stretch justify-center gap-4 max-[720px]:flex-col max-[720px]:w-full"
    >
      <GetStartedCard />
      <CodingWithAiCard />
    </section>
...
function GetStartedCard() {
  return () => (
    <div className="flex flex-none flex-col items-center gap-8 rounded-[20px] bg-surface-3 px-4 pb-4 pt-8">
      <h2 className="m-0 flex w-full items-start px-4 text-sm font-bold uppercase leading-normal tracking-[0.1em] text-text-primary">
        Get started
      </h2>
      <ul className="m-0 flex w-full list-none flex-col items-stretch p-0">
        <li>
          <CardLink
...
function CodingWithAiCard() {
  return () => (
    <div className="flex w-[540px] flex-col items-center justify-center gap-8 rounded-[20px] bg-surface-3 px-4 pb-4 pt-8 max-[720px]:w-full">
      <h2 className="m-0 flex w-full items-start px-4 text-sm font-bold uppercase leading-normal tracking-[0.1em] text-text-primary">
        Coding with AI?
      </h2>
      <div className="flex w-full flex-col items-stretch">
        <p className="m-0 px-4 pb-4 text-sm leading-relaxed text-text-primary">
...
function CardLink(
  handle: Handle<{ href: string; icon: RemixNode; label: string }>,
) {
  return () => {
    ...
    return (
      <a
        href={href}
        className="flex items-center gap-4 rounded-xl px-4 py-4 text-text-primary no-underline transition-[background-color,color] duration-150 ease-out hover:bg-surface-4 hover:text-brand-blue focus-visible:bg-surface-4 focus-visible:text-brand-blue focus-visible:outline-none"
      >
        <span aria-hidden="true" className="flex w-6 flex-none items-center">
          <span className="[&_svg]:block [&_svg]:h-5 [&_svg]:w-5">{icon}</span>
        </span>
    ...
    <footer className="flex flex-col items-center justify-center gap-2.5">
      <div className="flex items-center justify-center gap-6">
        ...
        <nav
          ...
          className="flex items-center justify-end gap-4 text-text-tertiary [&_a]:inline-flex [&_a]:h-5 [&_a]:w-5 [&_a]:items-center [&_a]:justify-center [&_a]:text-inherit [&_a]:transition-colors [&_a]:duration-150 [&_a]:ease-out hover:[&_a]:text-text-primary focus-visible:[&_a]:text-text-primary focus-visible:[&_a]:outline-none [&_svg]:block [&_svg]:h-full [&_svg]:w-full"
        >
      ...
      <div className="flex flex-col items-center justify-center gap-3 text-center text-[10px] leading-relaxed tracking-[0.05em] text-text-tertiary [&_p]:m-0 [&_p]:whitespace-nowrap">
...
function FooterWordmark() {
  return () => (
    <span
      role="img"
      aria-label="Remix"
      className="block h-2 w-[calc(8px*163/16)] text-text-primary opacity-55 [&_svg]:block [&_svg]:h-full [&_svg]:w-full"
    >
...
function RemixWordmarkHero() {
  return () => (
    <svg
      ...
      aria-label="Remix"
      viewBox="0 0 820 73"
      className="block h-auto w-full text-text-primary"
    >
      <path
