...
export function Layout(handle: Handle<LayoutProps>) {
  ...
  handle.queueTask(() => {
    handle.frame.addEventListener(
      ...
      () => {
        showLoading =
          new URL(handle.frame.src).pathname !== handle.props.url.pathname;
        void handle.update();
      },
...
export function HomePage() {
  return () => (
    <article>
      ...
      <p mix={bodyStyle}>
        The SPA runtime preserves the fetch router's <code>Request</code> to
        {" "}
        <code>Response</code>{" "}
        contract while route handlers work directly with Remix nodes through
        {" "}
        <code>render()</code>.
      </p>
...
export function AboutPage() {
  return () => (
    <article>
      ...
      <p mix={bodyStyle}>
        Each route waits briefly before returning a SPA response, making
        top-frame loading and cancellation visible without introducing a second
        navigation system.
      </p>
    </article>
  );
...
export function GreetingPage(
  handle: Handle<{ isSubmission?: boolean; name: string }>,
) {
  let isPending = false;
  ...
  return () => (
    <article>
      ...
      <p mix={bodyStyle}>
        The frame navigation listener turns the native form navigation into a
        routed request. Back and forward traversals revisit this URL with GET
        because history entries do not retain form data.
      </p>
      <form
        method="POST"
      ...
      <p mix={bodyStyle}>
        This form targets the current URL, so it replaces the active history
        entry. The first submission from <a href={routes.home.href()}>home</a>
        {" "}
        pushes a new entry.
      </p>
    </article>
